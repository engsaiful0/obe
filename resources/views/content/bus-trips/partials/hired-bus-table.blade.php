                ...
                <tr data-bus-id="{{ $bus->id }}" data-sub-type="{{ $bus->busSubType->sub_type_name ?? '' }}" data-tab="{{ $tabPrefix }}">
                    ...
                    <td>
                        <select name="trips[{{ $uniqueIndex }}][start_stoppage_id]"
                            class="select2 form-select form-select-sm start-stoppage-select auto-stoppage-select"
                            data-index="{{ $uniqueIndex }}">
                            <option value="">Select Start Stoppage</option>
                            @foreach($stoppages as $stoppage)
                            @php
                                $selectedStart = old('trips.'.$uniqueIndex.'.start_stoppage_id', $defaultStoppages['start_stoppage_id'] ?? '');
                            @endphp
                            <option value="{{ $stoppage->id }}" {{ $selectedStart == $stoppage->id ? 'selected' : '' }}>
                        ...
                        <select name="trips[{{ $uniqueIndex }}][end_stoppage_id]"
                            class="select2 form-select form-select-sm end-stoppage-select mt-1 auto-stoppage-select"
                            data-index="{{ $uniqueIndex }}">
                            <option value="">Select End Stoppage</option>
                            @foreach($stoppages as $stoppage)
                            @php
                                $selectedEnd = old('trips.'.$uniqueIndex.'.end_stoppage_id', $defaultStoppages['end_stoppage_id'] ?? '');
                            @endphp
                            <option value="{{ $stoppage->id }}" {{ $selectedEnd == $stoppage->id ? 'selected' : '' }}>
                                {{ $stoppage->stoppage_name }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <td>
                        <input name="trips[{{ $uniqueIndex }}][passengers]"
                            type="number"
                    ...
                    <td>
                        <select style="width:100px" name="trips[{{ $uniqueIndex }}][trip_type]"
                            class=" form-select form-select-sm trip-type-select auto-trip-type"
                            data-index="{{ $uniqueIndex }}">
                    ...
                    @endif
                    <td>
                        <input  class="form-control form-control-sm distance-input" placeholder="TN" type="number" readonly  name="trips[{{ $uniqueIndex }}][trip_number]" value="{{ $busTripNumbers[$bus->id] ?? '' }}">
                    </td>
                    <td>
                        <button type="button" 
