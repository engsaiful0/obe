...
@section('title', 'Edit Driver & Helper Assignment')

@section('content')
<div class="card">
    <div class="card-header">
        <h5 class="card-title">Edit Driver & Helper Assignment</h5>
    </div>
    <div class="card-body">
        <!-- Alert Messages -->
        ...
        <form action="{{ route('driver-helper-assignments.update', $driverHelperAssignment) }}" method="POST" id="assignmentForm">
            @csrf
            @method('PUT')
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('bus_id') is-invalid @enderror" id="bus_id" name="bus_id" required>
                        ...
                        @foreach($buses as $bus)
                            <option value="{{ $bus->id }}" {{ old('bus_id', $driverHelperAssignment->bus_id) == $bus->id ? 'selected' : '' }}>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select select2 @error('driver_id') is-invalid @enderror" id="driver_id" name="driver_id" required>
                        ...
                        @foreach($drivers as $driver)
                            <option value="{{ $driver->id }}" {{ old('driver_id', $driverHelperAssignment->driver_id) == $driver->id ? 'selected' : '' }}>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select select2 @error('bus_helper_id') is-invalid @enderror" id="bus_helper_id" name="bus_helper_id" required>
                        ...
                        @foreach($busHelpers as $helper)
                            <option value="{{ $helper->id }}" {{ old('bus_helper_id', $driverHelperAssignment->bus_helper_id) == $helper->id ? 'selected' : '' }}>
            ...
            <div class="row mb-4">
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select select2 @error('status_id') is-invalid @enderror" id="status_id" name="status_id" required>
                        ...
                        @foreach($statuses as $status)
                            <option value="{{ $status->id }}" {{ old('status_id', $driverHelperAssignment->status_id) == $status->id ? 'selected' : '' }}>
                                {{ $status->status_name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="date" class="form-control @error('assignment_date') is-invalid @enderror" 
                           ...
                           value="{{ old('assignment_date', $driverHelperAssignment->assignment_date ? $driverHelperAssignment->assignment_date->format('Y-m-d') : '') }}" required>
            ...
            <div class="row mb-4">
                <div class="col-12 mb-3">
                    ...
                    <textarea class="form-control @error('notes') is-invalid @enderror" 
                              id="notes" name="notes" rows="3" placeholder="Optional notes about this assignment...">{{ old('notes', $driverHelperAssignment->notes) }}</textarea>
                    @error('notes')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-12">
                    <div class="d-flex justify-content-end gap-2">
                        ...
                        <button type="submit" class="btn btn-primary" id="submitBtn">
                            <i class="ti ti-check me-1"></i>Update Assignment
        ...
        $('#assignmentForm').on('submit', function(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        toastr.success(response.message || 'Assignment updated successfully!');
                        setTimeout(() => {
                            window.location.href = '{{ route("driver-helper-assignments.index") }}';
                        }, 1500);
                    } else {
                        toastr.error(response.message || 'Failed to update assignment.');
                    }
                },
                error: function(xhr) {
                    toggleFormState(false);
