...
<script>
    window.departmentUrls = {
        store: '{{ route('app-settings-department.store') }}',
        index: '{{ route('app-settings-department') }}'
    };
</script>
<script src="{{ asset('assets/js/department-add.js') }}?v={{ time() }}"></script>
@endsection
...
<div class="container-xxl flex-grow-1 container-p-y">
    <div class="card">
        ...
        <div class="card-body">
            <form id="department-form" method="POST" action="{{ route('app-settings-department.store') }}">
                @csrf
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="input-group input-group-merge">
                            ...
                            <select id="name" class="form-select @error('name') is-invalid @enderror" name="name" aria-label="Select Department" aria-describedby="name2" required>
                                ...
                                <option value="{{ $dept }}" {{ old('name') == $dept ? 'selected' : '' }}>
                                    {{ $dept }}
                                </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="input-group input-group-merge">
                            ...
                            <textarea id="description" class="form-control @error('description') is-invalid @enderror" 
                                      ...
                                      aria-label="Enter description" aria-describedby="description2">{{ old('description') }}</textarea>
                        </div>
                        @error('description')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
