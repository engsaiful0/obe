...
<div class="card">
    <div class="card-header">
        <h5 class="card-title">Edit Reward Record</h5>
    </div>
    <div class="card-body" data-bus-id="{{ $reward->bus_id }}" data-bus-sub-type-id="{{ $reward->bus_sub_type_id }}">
        <!-- Alert Messages -->
        ...
        <form action="{{ route('rewards.update', $reward) }}" method="POST" enctype="multipart/form-data" id="rewardForm">
            @csrf
            @method('PUT')
            ...
            <div class="row mb-4">
                <div class="col-12">
                    <h6 class="fw-bold text-primary mb-3"><i class="ti ti-info-circle me-2"></i>Basic Information</h6>
                </div>
            </div>
            ...
            <div class="row g-3 mb-4">
                <div class="col-md-3">
                    ...
                    <select name="bus_sub_type_id" id="bus_sub_type_id" class="select2 form-select @error('bus_sub_type_id') is-invalid @enderror" required>
                        ...
                        <option value="{{ $subType->id }}" data-name="{{ $subType->sub_type_name }}" {{ old('bus_sub_type_id', $reward->bus_sub_type_id) == $subType->id ? 'selected' : '' }}>
            ...
            <div class="row mb-4">
                <div class="col-12">
                    <h6 class="fw-bold text-primary mb-3"><i class="ti ti-gift me-2"></i>Reward Details</h6>
                </div>
            </div>
            ...
            <div class="row g-3 mb-4">
                <div class="col-md-4">
                    ...
                    <div class="input-group">
                        ...
                        <input type="number" step="0.01" min="0" name="reward_amount" id="reward_amount" class="form-control"
                            value="{{ old('reward_amount', $reward->reward_amount) }}" placeholder="0.00" required>
                    </div>
                </div>
                <div class="col-md-4">
                    <label for="reward_date" class="form-label">Reward Date <span class="text-danger">*</span></label>
                    <input type="date" name="reward_date" id="reward_date" class="form-control"
            ...
            <div class="row g-3 mb-4">
                <div class="col-md-4">
                    ...
                    <textarea name="reason" id="reason" rows="4" class="form-control" required>{{ old('reason', $reward->reason) }}</textarea>
                </div>

                <div class="col-md-4">
                    <label for="remarks" class="form-label">Remarks</label>
                    <textarea name="remarks" id="remarks" rows="3" class="form-control">{{ old('remarks', $reward->remarks) }}</textarea>
                </div>
                <div class="col-md-4">
                    ...
                    <select name="witness_employee_id" id="witness_employee_id" class="select2 form-select">
                        ...
                        <option value="{{ $witness_employee->id }}" {{ old('witness_employee_id', $reward->witness_employee_id) == $witness_employee->id ? 'selected' : '' }}>
            ...
            <div class="row mb-4">
                <div class="col-12">
                    <h6 class="fw-bold text-primary mb-3"><i class="ti ti-paperclip me-2"></i>Supporting Document</h6>
                </div>
            </div>
            ...
            <div class="row g-3 mb-4">
                <div class="col-md-6">
                    ...
                    <div class="mb-2">
                        ...
                        <a href="{{ asset('reward_documents/' . $reward->document) }}" target="_blank" class="text-primary">
                            {{ $reward->document }}
                        </a>
                    </div>
                    @endif
                ...
                <div class="col-12">
                    <div class="d-flex justify-content-end gap-2">
                        ...
                        <button type="submit" class="btn btn-primary" id="submitBtn">
                            <i class="ti ti-check me-1"></i>Update Reward
                        </button>
                    </div>
                </div>
            </div>
...
<script>
    $(document).ready(function() {
        // Store current bus_id and bus_sub_type_id for initial load
        const cardBody = $('.card-body');
        const currentBusId = cardBody.data('bus-id') || null;
        const currentBusSubTypeId = cardBody.data('bus-sub-type-id') || null;
        ...
        function setLoading(loading) {
            ...
            if (loading) {
                ...
                // Restore button text
                submitBtn.html('<i class="ti ti-check me-1"></i>Update Reward');
            }
        }
        ...
        $('#rewardForm').on('submit', function(e) {
            ...
            // AJAX request to update reward
            $.ajax({
                url: '{{ route("rewards.update", $reward) }}',
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        ...
                        showAlert('success', response.message || 'Reward updated successfully!');
                        
                        // Show toastr notification
                        if (typeof toastr !== 'undefined') {
                            toastr.success(response.message || 'Reward updated successfully!');
                        }
                        ...
                        // Show error message
                        showAlert('danger', response.message || 'Failed to update reward.');
                        
                        if (typeof toastr !== 'undefined') {
                            toastr.error(response.message || 'Failed to update reward.');
                        }
                    }
                },
                error: function(xhr) {
