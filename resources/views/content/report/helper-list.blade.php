...
<script>
  $(function () {
    ...
    $('#helper-list-form').on('submit', function(e) {
      e.preventDefault();
      filterHelpers();
    });
    ...
    function filterHelpers() {
      var formData = $('#helper-list-form').serialize();
      
      $.ajax({
        url: '{{ route("helper-list.ajax") }}',
        type: 'POST',
        data: formData,
        headers: {
        ...
        beforeSend: function() {
          $('#helper-list-table-container').html('<div class="text-center py-4"><div class="spinner-border text-primary" role="status"><span class="visually-hidden">Loading...</span></div></div>');
        },
        success: function(response) {
          if (response.success) {
            $('#helper-list-table-container').html(response.html);
            
            // Update pagination if exists
            if (response.pagination) {
              $('#helper-list-table-container').find('.card-footer').remove();
              $('#helper-list-table-container').append(response.pagination);
            }
            ...
            $('a[href*="helper-list/pdf"]').attr('href', '{{ route("helper-list.pdf") }}?' + queryString);
            $('a[href*="helper-list/excel"]').attr('href', '{{ route("helper-list.excel") }}?' + queryString);
            $('a[href*="helper-list/print"]').attr('href', '{{ route("helper-list.print") }}?' + queryString);
          }
        },
        error: function(xhr) {
          console.error('Error filtering helpers:', xhr.responseText);
          $('#helper-list-table-container').html('<div class="alert alert-danger">Error loading data. Please try again.</div>');
        }
      });
    }
  });
...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h5 class="card-title mb-0">Helper List</h5>
        <div>
            <a href="{{ route('helper-list.print', request()->query()) }}" class="btn btn-info" target="_blank">
                <i class="ti ti-printer me-1"></i> Print
            </a>
            <a href="{{ route('helper-list.pdf', request()->query()) }}" class="btn btn-danger" target="_blank">
                <i class="ti ti-file-pdf me-1"></i> Export to PDF
            </a>
            <a href="{{ route('helper-list.excel', request()->query()) }}" class="btn btn-success">
    ...
    <div class="card-body">
        ...
        <div class="row mb-4">
            <div class="col-md-4">
                <div class="card bg-label-primary">
                    <div class="card-body">
                        <h6 class="card-title text-primary">Total Helpers</h6>
        ...
        {{-- Search Form --}}
        <form id="helper-list-form" action="{{ route('helper-list') }}" method="GET">
            <div class="row g-3">
                <div class="col-md-3">
    ...
    <div id="helper-list-table-container">
        @include('content.report.helper-list-table', ['busHelpers' => $busHelpers, 'totalCount' => $totalCount, 'totalSalary' => $totalSalary, 'avgSalary' => $avgSalary])
    </div>
</div>
@endsection
