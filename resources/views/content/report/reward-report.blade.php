...
<script>
  $(function () {
    ...
    $('#reward-report-form').on('submit', function(e) {
      e.preventDefault();
      filterRewards();
    });
    ...
    function filterRewards() {
      var formData = $('#reward-report-form').serialize();
      
      $.ajax({
        url: '{{ route("reward-report.ajax") }}',
        type: 'POST',
        data: formData,
        headers: {
        ...
        beforeSend: function() {
          $('#reward-table-container').html('<div class="text-center"><div class="spinner-border" role="status"><span class="visually-hidden">Loading...</span></div></div>');
        },
        success: function(response) {
          $('#reward-table-container').html(response.html);
          
          // Update export links with current filters
          var queryString = new URLSearchParams(formData).toString();
          $('a[href*="reward-report.print-list"]').attr('href', '{{ route("reward-report.print-list") }}?' + queryString);
          $('a[href*="reward-report.pdf"]').attr('href', '{{ route("reward-report.pdf") }}?' + queryString);
          $('a[href*="reward-report.excel"]').attr('href', '{{ route("reward-report.excel") }}?' + queryString);
        },
        error: function(xhr) {
          console.error('Error filtering rewards:', xhr.responseText);
          $('#reward-table-container').html('<div class="alert alert-danger">Error loading data. Please try again.</div>');
        }
      });
    }
  });
...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h5 class="card-title mb-0">Reward Report</h5>
        <div class="d-flex gap-2">
            <a href="{{ route('reward-report.print-list', request()->query()) }}" 
               target="_blank" 
               class="btn btn-outline-primary">
                <i class="ti ti-printer"></i> Print
            </a>
            <a href="{{ route('reward-report.pdf', request()->query()) }}" class="btn btn-danger" target="_blank">
                <i class="ti ti-file-pdf"></i> Export to PDF
            </a>
            <a href="{{ route('reward-report.excel', request()->query()) }}" class="btn btn-success">
    ...
    <div class="card-body">
        {{-- Search Form --}}
        <form id="reward-report-form" action="{{ route('reward-report') }}" method="GET">
            <div class="row">
                <div class="col-md-2">
                    <label for="reward_type_id" class="form-label">Reward Type</label>
                    <select id="reward_type_id" name="reward_type_id" class="form-select">
                        <option value="">All</option>
                        @foreach ($rewardTypes as $type)
                        <option value="{{ $type->id }}" {{ request('reward_type_id') == $type->id ? 'selected' : '' }}>
                            {{ $type->name }}
                        </option>
                        @endforeach
                    </select>
    ...
    <div id="reward-table-container">
        @include('content.report.reward-report-table')
    </div>
</div>
@endsection
