...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h5 class="card-title mb-0">
            <i class="ti ti-gas-station me-2"></i>Add Fuel
        </h5>
    ...
    <div class="card-body">
        ...
        @if(count($buses) > 0)
            <form id="fuelForm">
                ...
                <div class="row mb-4">
                    <div class="col-md-4">
                        <label for="common_fuel_date" class="form-label">Date <span class="text-danger">*</span></label>
                        <input type="text" 
                               id="common_fuel_date" 
                               name="fuel_date" 
                               class="form-control" 
                    ...
                    <div class="col-md-4">
                        <label for="common_fuel_time" class="form-label">Time <span class="text-danger">*</span></label>
                        <input type="time" 
                               id="common_fuel_time" 
                               name="fuel_time" 
                               class="form-control" 
                    ...
                    <table class="table table-bordered table-striped">
                        <thead class="table-dark">
                            <tr>
                                ...
                                <th style="width: 15%;">Fuel Amount <span class="text-danger">*</span></th>
                            ...
                            @foreach($buses as $index => $bus)
                                <tr data-bus-id="{{ $bus->id }}">
                                    <td>
                                        ...
                                        <input type="hidden" name="fuels[{{ $index }}][bus_id]" value="{{ $bus->id }}" class="fuel-bus-id">
                                    </td>
                                    <td>
                                        <input type="number" 
                                               step="0.01" 
                                               min="0"
                                               class="form-control fuel-amount" 
                                               name="fuels[{{ $index }}][fuel_amount]" 
                                               placeholder="0.00"
                                               data-bus-id="{{ $bus->id }}">
                                        <div class="invalid-feedback"></div>
                                    </td>
                                    <td>
                                        <select class="form-select fuel-unit" 
                                                name="fuels[{{ $index }}][unit_id]" 
                                                data-bus-id="{{ $bus->id }}"
                                                required>
                                    ...
                                    <td>
                                        <textarea class="form-control fuel-comment" 
                                                  name="fuels[{{ $index }}][comment]" 
                                                  rows="2" 
                                                  data-bus-id="{{ $bus->id }}"
...
<script>
    $(document).ready(function() {
        ...
        $('.save-row-btn').on('click', function() {
            ...
            const formData = {
                bus_id: row.find('.fuel-bus-id').val(),
                fuel_date: $('#common_fuel_date').val(),
                fuel_time: $('#common_fuel_time').val(),
                concern_employee_id: $('#common_concern_employee_id').val(),
                fuel_amount: row.find('.fuel-amount').val(),
                unit_id: row.find('.fuel-unit').val(),
                comment: row.find('.fuel-comment').val(),
                _token: $('meta[name="csrf-token"]').attr('content')
            };

            // Validate
            if (!formData.fuel_date || !formData.fuel_time || !formData.fuel_amount || formData.fuel_amount <= 0 || !formData.unit_id) {
                showAlert('danger', 'Please fill in all required fields (Date, Time, Fuel Amount, and Unit).');
                return;
            }
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        ...
                        row.find('.fuel-amount').val('');
                        row.find('.fuel-comment').val('');
                    } else {
        ...
        $('#saveAllBtn').on('click', function() {
            ...
            // Collect all rows with fuel amount
            const fuels = [];
            $('tr[data-bus-id]').each(function() {
                const row = $(this);
                const fuelAmount = row.find('.fuel-amount').val();
                const busId = row.find('.fuel-bus-id').val();
                
                if (fuelAmount && fuelAmount > 0 && busId) {
                    fuels.push({
                        bus_id: busId,
            ...
            if (fuels.length === 0) {
                showAlert('warning', 'Please enter fuel amount for at least one bus.');
                return;
            }

            // Validate common fields
            const fuelDate = $('#common_fuel_date').val();
            const fuelTime = $('#common_fuel_time').val();
