...
@section('title', __('Edit BAC Standard'))

@section('content')
<div class="card">
    <div class="card-header">
        <h5 class="mb-0">{{ __('Edit BAC standard') }}</h5>
    </div>
    <div class="card-body">
        <form method="POST"
            action="{{ route('bac-standards.update', $standard) }}"
            ...
            @include('content.bac.standards._form', ['standard' => $standard])
            <div class="mt-3 d-flex gap-2">
                ...
                <a href="{{ route('bac-standards.index') }}" class="btn btn-outline-secondary">{{ __('Cancel') }}</a>
            </div>
        </form>
    </div>
</div>
