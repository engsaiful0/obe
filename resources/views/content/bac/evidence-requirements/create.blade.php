...
@section('title', __('Add BAC Evidence Requirement'))

@section('content')
<div class="card">
    <div class="card-header">
        <h5 class="mb-0">{{ __('New BAC evidence requirement') }}</h5>
    </div>
    <div class="card-body">
        <form method="POST"
            action="{{ route('bac-evidence-requirements.store') }}"
            data-async-no-submit
            autocomplete="off"
            onsubmit="return false;">
            @csrf
            <div class="alert alert-danger d-none mb-3" role="alert" data-ajax-errors></div>
            @include('content.bac.evidence-requirements._form', ['requirement' => null])
