...
@section('title', __('Add BAC Criterion'))

@section('content')
<div class="card">
    <div class="card-header">
        <h5 class="mb-0">{{ __('New BAC criterion') }}</h5>
    </div>
    <div class="card-body">
        <form method="POST"
            action="{{ route('bac-criteria.store') }}"
            data-async-no-submit
            autocomplete="off"
            onsubmit="return false;">
            @csrf
            <div class="alert alert-danger d-none mb-3" role="alert" data-ajax-errors></div>
            @include('content.bac.criteria._form', ['criterion' => null])
