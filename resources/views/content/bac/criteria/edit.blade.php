...
@section('title', __('Edit BAC Criterion'))

@section('content')
<div class="card">
    <div class="card-header">
        <h5 class="mb-0">{{ __('Edit BAC criterion') }}</h5>
    </div>
    <div class="card-body">
        <form method="POST"
            action="{{ route('bac-criteria.update', $criterion) }}"
            ...
            @include('content.bac.criteria._form', ['criterion' => $criterion])
            <div class="mt-3 d-flex gap-2">
                ...
                <a href="{{ route('bac-criteria.index') }}" class="btn btn-outline-secondary">{{ __('Cancel') }}</a>
            </div>
        </form>
    </div>
</div>
