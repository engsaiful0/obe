...
@section('title', __('Add PO / PLO'))

@section('content')
<div class="card">
    <div class="card-header">
        <h5 class="mb-0">{{ __('New program outcome') }}</h5>
    </div>
    <div class="card-body">
        <form method="POST"
            action="{{ route('program-outcomes.store') }}"
            id="program-outcome-create-form"
            data-async-no-submit
            autocomplete="off"
            onsubmit="return false;">
            @csrf
            <div class="alert alert-danger d-none mb-3" role="alert" data-ajax-errors></div>
            @include('content.program-outcomes._form', ['programOutcome' => null])
