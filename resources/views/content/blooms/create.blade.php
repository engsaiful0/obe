...
@section('title', __('Add Bloom level'))

@section('content')
<div class="card">
    <div class="card-header">
        <h5 class="mb-0">{{ __("Bloom's taxonomy — new level") }}</h5>
    </div>
    <div class="card-body">
        <form method="POST"
            action="{{ route('blooms.store') }}"
            id="bloom-create-form"
            data-async-no-submit
            autocomplete="off"
            onsubmit="return false;">
            @csrf
            <div class="alert alert-danger d-none mb-3" role="alert" data-ajax-errors></div>
            @include('content.blooms._form', ['bloom' => null])
            <div class="mt-3 d-flex gap-2">
                ...
                <a href="{{ route('blooms.index') }}" class="btn btn-outline-secondary">{{ __('Cancel') }}</a>
            </div>
        </form>
    </div>
</div>
