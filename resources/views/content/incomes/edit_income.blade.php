...
<script>
    window.incomeUrls = {
        update: '{{ route('app-incomes.update', $income->id) }}',
        index: '{{ route('app-incomes') }}'
    };
</script>
<script src="{{ asset('assets/js/income-edit.js') }}?v={{ time() }}"></script>
@endsection
...
<div class="container-xxl flex-grow-1 container-p-y">
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h5 class="card-title mb-0">Edit Income - ID: {{ $income->id }}</h5>
            <a href="{{ route('app-incomes') }}" class="btn btn-label-secondary">
                <i class="ti ti-arrow-left me-1"></i> Back to List
            </a>
        </div>
        <div class="card-body">
            <form id="income-form" method="POST" action="{{ route('app-incomes.update', $income->id) }}">
                @csrf
                @method('PUT')
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="input-group input-group-merge">
                            ...
                            <select id="income_head_id" class="form-select @error('income_head_id') is-invalid @enderror" name="income_head_id" aria-label="Select Income Head" aria-describedby="income_head_id2" required>
                                ...
                                <option value="{{ $incomeHead->id }}" {{ old('income_head_id', $income->income_head_id) == $incomeHead->id ? 'selected' : '' }}>
                                    {{ $incomeHead->name }}
                                </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="input-group input-group-merge">
                            ...
                            <input type="number" id="amount" class="form-control @error('amount') is-invalid @enderror" name="amount" 
                                   placeholder="Enter Amount" step="0.01" min="0" value="{{ old('amount', $income->amount) }}" 
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="input-group input-group-merge">
                            ...
                            <input type="date" id="income_date" class="form-control @error('income_date') is-invalid @enderror" 
                                   name="income_date" value="{{ old('income_date', $income->income_date ? $income->income_date->format('Y-m-d') : '') }}" 
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="input-group input-group-merge">
                            ...
                            <select id="employee_id" class="form-select @error('employee_id') is-invalid @enderror" 
                                ...
                                <option value="{{ $employee->id }}" {{ old('employee_id', $income->employee_id) == $employee->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="input-group input-group-merge">
                            ...
                            <textarea id="remarks" class="form-control @error('remarks') is-invalid @enderror" 
                                      ...
                                      aria-label="Enter Remarks" aria-describedby="remarks2">{{ old('remarks', $income->remarks) }}</textarea>
                        </div>
                        @error('remarks')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
