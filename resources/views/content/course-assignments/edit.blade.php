...
@section('title', __('Edit course assignment'))

@section('content')
<div class="card">
    <div class="card-header"><h5 class="mb-0">{{ __('Edit assignment') }}</h5></div>
    <div class="card-body">
        <div id="course-assignment-form-errors" class="alert alert-danger d-none" role="alert"></div>
        <form id="course-assignment-form" method="POST" action="{{ route('course-assignment.update', $assignment) }}">
            @csrf
            @method('PUT')
            @include('content.course-assignments._form', ['assignment' => $assignment])
            <div class="mt-3 d-flex gap-2">
                <button type="submit" id="course-assignment-save-btn" class="btn btn-primary d-inline-flex align-items-center gap-2">
                    <span class="btn-label">{{ __('Update assignment') }}</span>
...
@section('page-script')
@include('content.course-assignments._cascade_script', ['assignment' => $assignment])
<script>
(function () {
    var indexUrl = @json(route('course-assignment.index'));
    var fallbackErr = @json(__('Could not update assignment.'));
