...
class QuestionCloMappingController extends Controller
{
    ...
    protected function filteredQuery(Request $request)
    {
        ...
        foreach (['program_id', 'course_id', 'assessment_component_id', 'clo_id', 'bloom_id', 'status_id'] as $field) {
    ...
    {
        $question_clo_mapping->load(['program', 'course', 'assessmentComponent', 'clo', 'bloom', 'status']);
    ...
    public function matrix(Request $request): View
    {
        ...
        if ($programId && $courseId) {
            ...
            $q = QuestionCloMapping::query()
                ->with(['assessmentComponent', 'clo', 'bloom'])
            ...
            if ($assessmentComponentId) {
                ...
                if ($selectedComponent && (int) $selectedComponent->course_id === $courseId) {
                    ...
                    $mappedTotal = QuestionCloMapping::sumMarksForComponent($assessmentComponentId, null);
                    $remaining = round($componentCap - $mappedTotal, 2);
                }
            }
        }
