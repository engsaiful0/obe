  ...
  public function index()
  {
    ...
    try {
      $busStats = [
        ...
        'maintenance' => Bus::where('status', 'under_maintenance')->count(),
        'inactive' => Bus::where('status', 'inactive')->count(),
        'this_month' => Bus::where('created_at', '>=', $thisMonth)->count(),
        'this_year' => Bus::where('created_at', '>=', $thisYear)->count(),
      ];
    } catch (\Exception $e) {
    ...
    // ============ ASSISTANT MODULE STATISTICS ============
    $assistantQuery = Assistant::query();
    if ($userId) {
      $assistantQuery->where(function($q) use ($userId) {
        $q->where('user_id', $userId)->orWhereNull('user_id');
      });
    }
    
    $assistantStats = [
      'total' => (clone $assistantQuery)->count(),
      'active' => (clone $assistantQuery)->where('status', 'active')->count(),
      'inactive' => (clone $assistantQuery)->where('status', 'inactive')->count(),
      'this_month' => (clone $assistantQuery)->where('created_at', '>=', $thisMonth)->count(),
    ];
    ...
    $damageStats = [
      ...
      'total_quantity' => DB::table('damages')
        ...
        ->where(function($q) use ($userId) {
          if ($userId) {
            $q->where('damages.user_id', $userId)->orWhereNull('damages.user_id');
          }
        })
    ...
    try {
      ...
      if ($userId) {
        $scheduleQuery->where(function($q) use ($userId) {
          $q->where('user_id', $userId)->orWhereNull('user_id');
        });
      }
    ...
    if ($userId) {
      $itemQuery->where(function($q) use ($userId) {
        $q->where('user_id', $userId)->orWhereNull('user_id');
      });
      $warehouseQuery->where(function($q) use ($userId) {
        $q->where('user_id', $userId)->orWhereNull('user_id');
      });
    }
    ...
    try {
      ...
      if ($userId) {
        $monthlyBillQuery->where(function($q) use ($userId) {
          $q->where('user_id', $userId)->orWhereNull('user_id');
        });
      }
    ...
    try {
      ...
      if ($userId) {
        $supplierQuery->where(function($q) use ($userId) {
          $q->where('user_id', $userId)->orWhereNull('user_id');
        });
      }
    ...
    return view('content.dashboard.dashboards-analytics', compact(
      'busStats', 'busByType',
      'driverStats', 'assistantStats', 'employeeStats',
      'purchaseStats', 'purchaseMonthlyTrend', 'purchaseGrowth',
      'issueStats', 'issueItemsStats',
      'damageStats',
      'expenseStats', 'expenseMonthlyTrend', 'expenseByCategory', 'expenseGrowth',
      'rewardStats', 'punishmentStats',
