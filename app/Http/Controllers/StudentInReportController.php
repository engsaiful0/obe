...
class StudentInReportController extends Controller
{
    /**
    ...
    private function processTrips($trips)
    {
        $reportData = [];
            ...
            foreach ($trips as $trip) {
                if (!$trip->startStoppage || !$trip->bus) {
                    continue;
                }
                
                // Normalize stoppage name (trim and lowercase for comparison)
                $stoppageName = trim($trip->startStoppage->stoppage_name);
    ...
    public function index(Request $request)
    {
        ...
        // Get report data - group by stoppage name only, sum all students and collect all bus numbers
        $reportData = [];
        
        if ($fromDate && $toDate) {
            // Get all IN trips within the date range
            $trips = BusTrip::where('trip_type', 'in')
                ->whereBetween('trip_date', [$fromDate, $toDate])
                ->with(['startStoppage', 'bus.busSubType', 'busSubType'])
                ->get();
        ...
        return view('content.report.student-in-report', compact(
            'reportData',
            'stoppages',
            'fromDate',
            'toDate',
    ...
    public function print(Request $request)
    {
        ...
        return view('content.report.student-in-report-print', compact(
            'reportData',
            'fromDate',
            'toDate',
            'appSettings'
    ...
    public function pdf(Request $request)
    {
        ...
        $pdf = Pdf::loadView('content.report.student-in-report-pdf', compact(
            'reportData',
            'fromDate',
            'toDate',
            'appSettings'
        ...
        return $pdf->download('student-in-report-' . $fromDate . '-to-' . $toDate . '.pdf');
    }
    ...
    public function excel(Request $request)
    {
        ...
        $exportData = [];
        $exportData[] = ['Start Stoppage Name', 'Bus Number', 'Bus Sub Type', 'No of Student'];
        ...
        $filename = 'student-in-report-' . $fromDate . '-to-' . $toDate . '.csv';
        $headers = [
