...
class UpdateVisionRequest extends FormRequest
{
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($v) {
            ...
            $routeVision = $this->route('vision');
            $ignoreId = $routeVision instanceof Vision ? $routeVision->getKey() : null;

            $q = Vision::query()->where('type', $this->input('type'))->where('status_id', $statusId);
            if ($this->input('type') === 'University') {
            ...
            if ($q->exists()) {
                $v->errors()->add(
                    'status_id',
                    __('Only one active vision is allowed for this university or department.')
                );
            }
        });
    }
