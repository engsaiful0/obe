/**
 * DataTables Basic for Violation Types
 */
    ...
    fv = FormValidation.formValidation(formAddNewRecord, {
      fields: {
        name: {
          validators: {
            notEmpty: {
              message: 'The violation type name is required'
            }
          }
        }
      },
...
$(function () {
  ...
  if (dt_basic_table.length) {
    dt_basic = dt_basic_table.DataTable({
      ajax: {
        url: window.violationTypeUrls.getData,
        type: 'GET',
        dataSrc: 'data'
      },
      columns: [
      ...
      columnDefs: [
        {
          ...
          render: function (data, type, full, meta) {
            return (
              ...
              '<a href="javascript:;" class="btn btn-sm btn-text-secondary rounded-pill btn-icon violation-type-edit"><i class="ti ti-pencil ti-md"></i></a>' +
    ...
    $('div.head-label').html('<h5 class="card-title mb-0">Violation Types</h5>');
  }
  ...
  fv.on('core.form.valid', function () {
    ...
    if ($new_name != '') {
      var url = window.violationTypeUrls.store;
      var method = 'POST';
      var message = 'Violation type added successfully.';
      var data = {
      ...
      if (id) {
        url = window.violationTypeUrls.update + '/' + id;
        method = 'PUT';
  ...
  $('.datatables-basic tbody').on('click', '.delete-record', function () {
    ...
    Swal.fire({
      ...
      if (result.value) {
        $.ajax({
          url: window.violationTypeUrls.destroy + '/' + data.id,
          type: 'DELETE',
          data: {
            _token: $('meta[name="csrf-token"]').attr('content')
          },
          success: function (response) {
            row.remove().draw();
            toastr.success('Violation type deleted successfully.');
          },
          error: function (error) {
            if (error.responseJSON && error.responseJSON.errors) {
              var errors = error.responseJSON.errors;
  ...
  // Edit Record
  $('.datatables-basic tbody').on('click', '.violation-type-edit', function () {
    var row = dt_basic.row($(this).parents('tr'));
    var data = row.data();
