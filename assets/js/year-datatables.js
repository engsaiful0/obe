    ...
    fv = FormValidation.formValidation(formAddNewRecord, {
      fields: {
        year_name: {
          validators: {
            notEmpty: {
              message: 'The year name is required'
            }
          }
        }
      },
...
$(function () {
  ...
  if (dt_basic_table.length) {
      ...
      responsive: {
        details: {
          display: $.fn.dataTable.Responsive.display.modal({
            header: function (row) {
              var data = row.data();
              return 'Details of ' + data['year_name'];
            }
          }),
          type: 'column',
    ...
    $('div.head-label').html('<h5 class="card-title mb-0">Years</h5>');
  }
  ...
  fv.on('core.form.valid', function () {
    ...
    if ($new_name != '') {
      var url = window.yearUrls.store;
      var method = 'POST';
      var message = 'Year added successfully.';
      var data = {
  ...
  $('.datatables-basic tbody').on('click', '.delete-record', function () {
    ...
    Swal.fire({
      ...
      if (result.value) {
        $.ajax({
          url: window.yearUrls.destroy + '/' + data.id,
          type: 'DELETE',
          data: {
            _token: $('meta[name="csrf-token"]').attr('content')
          },
  ...
  $('.datatables-basic tbody').on('click', '.item-edit', function () {
    ...
    document.querySelector('.dt-full-name').value = data.year_name;
