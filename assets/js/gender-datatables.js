...
$(function () {
  ...
  if (dt_basic_table.length) {
      ...
      columnDefs: [
        {
          ...
          render: function (data, type, full, meta) {
            return (
              ...
              '<a href="javascript:;" class="btn btn-sm btn-text-secondary rounded-pill btn-icon gender-edit"><i class="ti ti-pencil ti-md"></i></a>' +
      ...
      responsive: {
        details: {
          display: $.fn.dataTable.Responsive.display.modal({
            header: function (row) {
              var data = row.data();
              return 'Details of ' + data['gender_name'];
            }
          }),
          type: 'column',
    ...
    $('div.head-label').html('<h5 class="card-title mb-0">Genders</h5>');
  }
  ...
  fv.on('core.form.valid', function () {
    ...
    if ($new_name != '') {
      var url = window.genderUrls.store;
      var method = 'POST';
      var message = 'Gender added successfully.';
      var data = {
      ...
      if (id) {
        url = window.genderUrls.update + '/' + id;
        method = 'PUT';
  ...
  $('.datatables-basic tbody').on('click', '.delete-record', function () {
    ...
    Swal.fire({
      ...
      if (result.value) {
        $.ajax({
          url: window.genderUrls.destroy + '/' + data.id,
          type: 'DELETE',
          data: {
            _token: $('meta[name="csrf-token"]').attr('content')
          },
  ...
  // Edit Record
  $('.datatables-basic tbody').on('click', '.gender-edit', function () {
    var row = dt_basic.row($(this).parents('tr'));
    var data = row.data();
    offCanvasEl = new bootstrap.Offcanvas(document.querySelector('#add-new-record'));
    document.querySelector('.dt-full-name').value = data.gender_name;
