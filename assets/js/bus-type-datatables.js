    ...
    fv = FormValidation.formValidation(formAddNewRecord, {
      fields: {
        bus_type_name: {
          validators: {
            notEmpty: {
              message: 'The bus type name is required'
            }
          }
        }
      },
...
$(function () {
  ...
  if (dt_basic_table.length) {
    dt_basic = dt_basic_table.DataTable({
      ajax: {
        url: window.busTypeUrls.getData,
        type: 'GET',
        dataSrc: 'data'
      },
      columns: [
      ...
      responsive: {
        details: {
          display: $.fn.dataTable.Responsive.display.modal({
            header: function (row) {
              var data = row.data();
              return 'Details of ' + data['bus_type'];
            }
          }),
          type: 'column',
    ...
    $('div.head-label').html('<h5 class="card-title mb-0">Bus Types</h5>');
  }
  ...
  fv.on('core.form.valid', function () {
    ...
    if ($new_name != '') {
      var url = window.busTypeUrls.store;
      var method = 'POST';
      var message = 'Bus Type added successfully.';
      var data = {
        _token: $('meta[name="csrf-token"]').attr('content'),
        bus_type_name: $new_name
      };

      if (id) {
        url = window.busTypeUrls.update + '/' + id;
        method = 'PUT';
