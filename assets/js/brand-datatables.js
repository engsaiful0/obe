...
$(function () {
  ...
  if (dt_basic_table.length) {
    ...
    $('div.head-label').html('<h5 class="card-title mb-0">Brands</h5>');
  }
  ...
  fv.on('core.form.valid', function () {
    ...
    if ($new_name != '') {
      var url = window.brandUrls.store;
      var method = 'POST';
      var message = 'Brand added successfully.';
      var data = {
      ...
      if (id) {
        url = window.brandUrls.update + '/' + id;
        method = 'PUT';
  ...
  $('.datatables-basic tbody').on('click', '.delete-record', function () {
    ...
    Swal.fire({
      ...
      if (result.value) {
        $.ajax({
          url: window.brandUrls.destroy + '/' + data.id,
          type: 'DELETE',
          data: {
            _token: $('meta[name="csrf-token"]').attr('content')
          },
  ...
  $('.datatables-basic tbody').on('click', '.item-edit', function () {
    ...
    document.querySelector('.dt-full-name').value = data.brand_name;
