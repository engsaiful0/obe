/**
 * Deployment Type CRUD with AJAX and Spinner
    ...
    setTimeout(() => {
      ...
      if (newRecord) {
        newRecord.addEventListener('click', function () {
          ...
          // Update modal title
          document.querySelector('#exampleModalLabel').textContent = 'New Deployment Type';
    ...
    fv = FormValidation.formValidation(formAddNewRecord, {
      fields: {
        deployment_type_name: {
          validators: {
            notEmpty: {
              message: 'The deployment type name is required'
            }
          }
        }
      },
...
$(function () {
  ...
  if (dt_basic_table.length) {
    dt_basic = dt_basic_table.DataTable({
      ...
      serverSide: false,
      ajax: {
        url: window.deploymentTypeUrls.getData,
        type: 'GET',
        dataSrc: 'data',
        error: function(xhr, error, thrown) {
          console.error('DataTable AJAX Error:', {xhr, error, thrown});
          if (typeof toastr !== 'undefined') {
            toastr.error('Failed to load deployment types. Please try again.');
          }
        }
      },
      columns: [
      ...
      columnDefs: [
        {
          ...
          render: function (data, type, full, meta) {
            return (
              ...
              '<a href="javascript:;" class="btn btn-sm btn-text-secondary rounded-pill btn-icon deployment-type-edit"><i class="ti ti-pencil ti-md"></i></a>' +
      ...
      responsive: {
        details: {
          display: $.fn.dataTable.Responsive.display.modal({
            header: function (row) {
              var data = row.data();
              return 'Details of ' + data['deployment_type_name'];
            }
          }),
          type: 'column',
    ...
    $('div.head-label').html('<h5 class="card-title mb-0">Deployment Types</h5>');
  }
  ...
  fv.on('core.form.valid', function () {
    ...
    if ($new_name != '') {
      var url = window.deploymentTypeUrls.store;
      var method = 'POST';
      var message = 'Deployment Type added successfully.';
      var data = {
        _token: $('meta[name="csrf-token"]').attr('content'),
        deployment_type_name: $new_name
      };

      if (id) {
        url = window.deploymentTypeUrls.update + '/' + id;
        method = 'PUT';
  ...
  $('.datatables-basic tbody').on('click', '.delete-record', function () {
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        AjaxUtils.request({
          url: window.deploymentTypeUrls.destroy + '/' + data.id,
          type: 'DELETE',
          data: {
            _token: $('meta[name="csrf-token"]').attr('content')
          },
          ...
          onSuccess: function (response) {
            ...
            if (typeof toastr !== 'undefined') {
              toastr.success('Deployment Type deleted successfully.');
            }
  ...
  // Edit Record
  $('.datatables-basic tbody').on('click', '.deployment-type-edit', function () {
    var row = dt_basic.row($(this).parents('tr'));
    var data = row.data();
    var $editBtn = $(this);
    ...
    setTimeout(function() {
      ...
      document.querySelector('.dt-full-name').value = data.deployment_type_name;
      $('#form-add-new-record').attr('data-id', data.id);
      // Update modal title
      document.querySelector('#exampleModalLabel').textContent = 'Edit Deployment Type';
      offCanvasEl.show();
