    ...
    fv = FormValidation.formValidation(formAddNewRecord, {
      fields: {
        blood_group_name: {
          validators: {
            notEmpty: {
              message: 'The blood group name is required'
            }
          }
        }
      },
...
$(function () {
  ...
  if (dt_basic_table.length) {
    dt_basic = dt_basic_table.DataTable({
      ajax: {
        url: window.bloodGroupUrls.getData,
        type: 'GET',
        dataSrc: 'data'
      },
      columns: [
      ...
      columnDefs: [
        {
          ...
          render: function (data, type, full, meta) {
            return (
              ...
              '<a href="javascript:;" class="btn btn-sm btn-text-secondary rounded-pill btn-icon blood-group-edit"><i class="ti ti-pencil ti-md"></i></a>' +
      ...
      responsive: {
        details: {
          display: $.fn.dataTable.Responsive.display.modal({
            header: function (row) {
              var data = row.data();
              return 'Details of ' + data['blood_group_name'];
            }
          }),
          type: 'column',
    ...
    $('div.head-label').html('<h5 class="card-title mb-0">Blood Groups</h5>');
  }
  ...
  fv.on('core.form.valid', function () {
    ...
    if ($new_name != '') {
      var url = window.bloodGroupUrls.store;
      var method = 'POST';  
      var message = 'Blood Group added successfully.';
      var data = {
        _token: $('meta[name="csrf-token"]').attr('content'),
        blood_group_name: $new_name
      };

      if (id) {
        url = window.bloodGroupUrls.update + '/' + id;
        method = 'PUT';
  ...
  $('.datatables-basic tbody').on('click', '.delete-record', function () {
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        $.ajax({
          url: window.bloodGroupUrls.destroy + '/' + data.id,
          type: 'DELETE',
          data: {
            _token: $('meta[name="csrf-token"]').attr('content')
          },
  ...
  // Edit Record
  $('.datatables-basic tbody').on('click', '.blood-group-edit', function () {
    var row = dt_basic.row($(this).parents('tr'));
    var data = row.data();
    offCanvasEl = new bootstrap.Offcanvas(document.querySelector('#add-new-record'));
    document.querySelector('.dt-full-name').value = data.blood_group_name;
